...
namespace AESCrypto
{
    public class EncryptedString
    {
        ...
        public static string EncryptString(string plainSourceStringToEncrypt, string passPhrase)
        {
            ...
            using (AesCryptoServiceProvider acsp = GetProvider(Encoding.Default.GetBytes(passPhrase),PaddingMode.PKCS7))
            {
        ...
        public static string DecryptString(string base64StringToDecrypt, string passphrase, PaddingMode mode)
        {
            //Set up the encryption objects
            using (AesCryptoServiceProvider acsp = GetProvider(Encoding.Default.GetBytes(passphrase),mode))
            {   
        ...
        private static AesCryptoServiceProvider GetProvider(byte[] key, PaddingMode mode)
        {
